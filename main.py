...
class UserProfileManager:
    ...
    async def create_user_profile(self, user_id: str, parent: ParentProfile, child: ChildProfile, system_prompt: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            system_prompts[user_id] = system_prompt
    ...
    async def get_user_profile(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if doc.exists:
                ...
                if user_id not in system_prompts and 'system_prompt' in profile:
                    system_prompts[user_id] = profile['system_prompt']
                return profile
            return None
...
async def register_user(request: UserRegistration):
    ...
    try:
        ...
        # Create user profile
        profile = await user_profile_manager.create_user_profile(
            user_id=user_id,
...
@app.get("/auth/profile/{firebase_token}")
async def get_user_profile_endpoint(firebase_token: str):
...
@app.put("/auth/profile", response_model=AuthResponse)
async def update_user_profile_endpoint(request: UserProfileUpdate):
    ...
    try:
        ...
        # Update user profile
        updated_profile = await user_profile_manager.update_user_profile(
            user_id=user_id,
            parent=request.parent,
            child=request.child,
            system_prompt=request.system_prompt
...
@app.delete("/auth/profile/{firebase_token}")
...
async def verify_token_endpoint(firebase_token: str):
    """Verify Firebase token and return user info"""
    try:
        user_info = await verify_firebase_token(firebase_token)
        
        # Get user profile if exists
        profile = await user_profile_manager.get_user_profile(user_info['uid'])
...
# EXISTING API ENDPOINTS (Updated system prompt endpoint)
